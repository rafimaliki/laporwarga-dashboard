...
const STATUS_CONFIG: Record<string, { label: string; className: string }> = {
  submitted: { label: "Terkirim", className: "bg-slate-100 text-slate-700" },
  verified: { label: "Terverifikasi", className: "bg-blue-100 text-blue-700" },
  in_progress: {
    label: "Dalam Proses",
...
export default function RecentReports() {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      <div className="p-6 border-b border-slate-100 flex">
        <div className="h-10 w-10 rounded-xl bg-sky-50 flex items-center justify-center text-sky-600">
          <FileText size={20} />
        </div>
        <div>
          <h3 className="font-semibold text-slate-800 ml-5">Laporan Terbaru</h3>
          <p className="text-sm text-slate-500 ml-5">
            Daftar laporan terbaru yang masuk
          </p>
        </div>
      </div>
